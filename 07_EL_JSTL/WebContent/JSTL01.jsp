...
%>
	...
	<p>page test(c:out) : <c:out value="${pageScope.test }"/></p> <%--out 잘 사용하지 않는다 --%>
	...
	<c:if test="${10 > 20 }">
		<p> 10 &gt; 20 결과가  true인 경우 실행문장</p>
	</c:if>
	<c:if test="${10 <= 20 }">
		<p>10 &le; 20 결과가  true인 경우 실행문장</p>
	</c:if>
	<hr>
	
	<h3>JSTL : if 태그2</h3>
	<c:set var="n1" value="10"/> <%--주의!!! 문자열 처리됨 --%>
...
<%
	...
	//JSTL, EL에서 사용할 수 있게 현재 page에서 사용할 수 있도록 scope에 저장
...
%>
	...
	<c:forEach var="name" items="${attr_names }"> <%--표현식 작성시: <%=arr%>, EL 작성시: ${attr_names}--%>
		<li>${name }</li>
	</c:forEach>
	<hr><hr>
