...
<%--현재 페이지에 표시할 데이터를 화면에 표시
	...
	2. 페이징 객체에 설정할 데이터 생성 저장
	3. (DB) 현재 페이지에 표시할 게시글 데이터 조회
...
<%	
	...
	//1. 전체 게시물 수량 구하기
	//System.out.println("total : " + BbsDAO.getTotCount()); 
	p.setTotalRecord(BbsDAO.getTotCount());
	p.setTotalPage(); //자동 계산 처리
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="vo" items="${list }">
					<tr>
						<td>${vo.bbsIdx }</td>
						<td class="align-left"><a href="view.jsp?bbsIdx=${vo.bbsIdx }&cPage=${pvo.nowPage}">${vo.subject }</a></td>
						<td>${vo.writer }</td>
